...
// source: routing/v1alpha2/traffic_policy.proto
...
// TrafficPolicy provides a custom routing policy based on the destination
// service and its subsets/versions, the source of the traffic as well as
// connection and request metadata.  The policy must provide a set of
// conditions for each protocol (TCP, UDP, HTTP) that the destination
// service exposes on its ports.
//
// The policy applies only to the ports on the destination service for which
// it provides protocol-specific match condition, e.g. if the policy does not
// specify a TCP condition, the policy does not apply to TCP traffic.
//
// For example, the following policy routes all traffic by default to pods of
...
// policy will be applied at the gateway named "bookinfo" as well as at all
...
//           version: v2
//
// A host name can be defined by only one TrafficPolicy.  A single traffic policy
// can be used to describe traffic properties for multiple HTTP and TCP
// ports.
type TrafficPolicy struct {
	// REQUIRED. The destination address for traffic captured by this traffic
	// policy.  Could be a DNS name with wildcard prefix or a CIDR
	...
	// where policies are being applied. Thus, when a client in the "default"
	// namespace applies a policy containing a name "reviews, Istio will setup
	...
	// single traffic policy is used for sidecars inside the mesh as well
	...
	// will be used, which would apply the policy to all sidecars in the
	// mesh. If a list of gateway names is provided, the policy will apply
	// only to the gateways. To apply a policy to both gateways and sidecars,
	...
	// Policies to apply (load balancing policy, connection pool
	// sizes, outlier detection).
	DestinationPolicy *DestinationPolicy `protobuf:"bytes,5,opt,name=destination_policy,json=destinationPolicy" json:"destination_policy,omitempty"`
	// One or more named sets that represent individual versions of a
	// service. DestinationPolicies can be overridden at subset level.
	Subsets []*Subset `protobuf:"bytes,6,rep,name=subsets" json:"subsets,omitempty"`
}
...
func (*TrafficPolicy) Descriptor() ([]byte, []int) { return fileDescriptor3, []int{0} }

func (m *TrafficPolicy) GetHosts() []string {
	if m != nil {
		return m.Hosts
	}
	return nil
...
func (m *TrafficPolicy) GetGateways() []string {
	if m != nil {
		return m.Gateways
	}
	return nil
...
func (m *TrafficPolicy) GetHttp() []*HTTPRoute {
	if m != nil {
		return m.Http
	}
	return nil
...
func (m *TrafficPolicy) GetTcp() []*TCPRoute {
	if m != nil {
		return m.Tcp
	}
	return nil
...
func (m *TrafficPolicy) GetDestinationPolicy() *DestinationPolicy {
	if m != nil {
...
func (m *TrafficPolicy) GetSubsets() []*Subset {
	if m != nil {
		return m.Subsets
	}
	return nil
...
// request/connection will be sent after processing a traffic policy. The
...
// associated with the traffic policy will be used to identify the service as
// <servicename>.<policynamespace>. However, if the service name contains
...
//           version: v2
//
// The following policy sets a timeout of 5s for all calls to
// productpage.prod service.  Notice that there are no subsets defined in
// this policy. Istio will fetch all instances of productpage.prod service
...
//
// The following policy sets a timeout of 5s for all calls to the external
...
type Destination struct {
	...
	// platform specific manner.  For example in Kubernetes, when a policy with
	...
	// reviews.bookinfo.svc.cluster.local. If the policy refers to the
	...
	// within the mesh. The corresponding subset must be defined traffic policy.
...
// gRPC traffic. See TrafficPolicy for usage examples.
type HTTPRoute struct {
	// Match conditions to be satisfied for the traffic policy to be
	// activated. All conditions inside a single match block have AND
	// semantics, while the list of match blocks have OR semantics. The policy
	// is matched if any one of the match blocks succeed.
	Match []*HTTPMatchRequest `protobuf:"bytes,1,rep,name=match" json:"match,omitempty"`
	// A http policy can either redirect or forward (default) traffic.  The
	// forwarding target can be one of several versions of a service (see
	// glossary in beginning of document). Weights associated with the
	// service version determine the proportion of traffic it receives.
	Route []*DestinationWeight `protobuf:"bytes,2,rep,name=route" json:"route,omitempty"`
	// A http policy can either redirect or forward (default) traffic.  If
	// traffic passthrough option is specified in the traffic policy,
	...
	// subject to further traffic policies based on the protocol selected.
...
// following traffic policy forwards traffic arriving at port 2379 named
...
// traffic policy to be applied to the HTTP request. For example, the following
// traffic policy restricts the policy to match only requests where the URL path
...
type HTTPMatchRequest struct {
	...
	// One or more labels that constrain the applicability of a policy to
	// workloads with the given labels. If the traffic policy has a list of
	...
	// Names of gateways where the policy should be applied to. Gateway names
	// at the top of the policy (if any) are overridden. The gateway match is
...
// Each traffic policy is associated with one or more service versions (see
// glossary in beginning of document). Weights associated with the version
// determine the proportion of traffic it receives. For example, the
// following policy will route 25% of traffic for the "reviews" service to
...
type DestinationWeight struct {
	...
	// If there is only destination in a policy, the weight value is assumed to
	// be 100.
...
// the specified values. For example, the following traffic policy redirects
...
// example, the following policy sets the maximum number of retries to 3 when
...
// the following policy restricts cross origin requests to those originating
...
// policy.  Fault specification is part of a traffic policy. Faults include
// aborting the Http request from downstream service, and/or delaying
// proxying of requests. A fault policy MUST HAVE delay or abort or both.
//
...
func init() { proto.RegisterFile("routing/v1alpha2/traffic_policy.proto", fileDescriptor3) }
...
var fileDescriptor3 = []byte{
	...
	,
}
