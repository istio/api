...
// RouteRule for usage examples. The destination.name can be a short name
// or a fully qualified domain name from the service registry, a resolvable
// DNS name, a service name from the service registry prefixed with a
// subset name or an IP address. The order of inference is as follows:
//
// 1. If the name is a single word, a short name, a lookup in the service
// registry is performed to obtain instances of the service. If the lookup
// fails, the rule will be ignored.
//
// The following examples are based on the reviews service running on
// kubernetes, within prod namespace. They are applicable in other
// environments as well. If the name consists of multiple words separated
// by a dot (e.g., v1.reviews.prod, or reviews.prod), one of the following
// types of inference occurs.
//
// 2. Service registry and subset lookup. The first word will be
// considered as the subset name, and the rest of the string is considered
// as the name of the service in the service registry. If the service does
// not exist in the service registry, the search proceeds to step 3. If a
// DestinationRule with the service name exists, and contains the subset
// matching the subset name in the route rule, the search terminates. If
// the subset does not exist in the DestinationRule, the search fails and
// the rule will be ignored. The pods/VMs identified by the subset will be
// used as the load balancing pool for the destination traffic.  For
// example, if destination.name is v1.reviews.prod, and a DestinationRule
// is defined for the reviews.prod service with subsets v1, v2, the pods
// identified by the v1 subset will be considered as the final destination
// for the request/connection, if and only if reviews.prod exists in the
// service registry.
//
// 3. Service registry lookup only. The entire name is looked up in the
// service registry. If the lookup succeeds, the search terminates. The
// requests will be routed to any instance of the service in the mesh. For
// example, when looking up reviews.prod, step 2 (above) fails, but step 3
// succeeds in a kubernetes environment, since there exists a service
// called reviews in the prod namespace.
//
// 3. Runtime DNS lookup. If steps 2 and 3 fail, the name is considered as
// a DNS name that is not in the service registry (e.g.,
// wikipedia.org). The sidecar/gateway will resolve the DNS and load
// balance requests appropriately. See Envoy's strict_dns for details.
//
// For example, the following rule routes all traffic by default to pods of
// reviews service with label "version: v1" on a subset named v1, and some
// to v2.reviews, in a kubernetes environment.
//
//     apiVersion: config.istio.io/v1alpha2
//     kind: RouteRule
//     metadata:
...
var fileDescriptor2 = []byte{
	...
	,
}
