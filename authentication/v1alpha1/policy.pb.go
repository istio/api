...
type Jwt struct {
	...
	// List of conditions to decide if this JWT should be used to validate the
	// request. The JWT validation happens if any one of the conditions matched.
	// If the list is non-empty and none of the conditions matched, authentication
	// will skip the JWT validation.
	// Leave this empty to always trigger the JWT validation.
	Conditions []*Condition `protobuf:"bytes,9,rep,name=conditions" json:"conditions,omitempty"`
}
...
func (m *Jwt) GetConditions() []*Condition {
	if m != nil {
		return m.Conditions
	}
	return nil
...
// Condition to match against a request. The condition is matched if and only if
// both excluded_paths and included_paths are satisfied.
type Condition struct {
	// List of paths to be excluded from the request. The excluded_paths is satisfied
	// if either the excluded_paths is empty or the request path is *NOT* matched
	// to any of the excluded_paths.
	...
	// List of paths that the request must include. The included_paths is satisfied
	// if either the included_paths is empty or the request path is matched to any
	// of the included_paths.
...
func (*Condition) Descriptor() ([]byte, []int) { return fileDescriptorPolicy, []int{2} }

func (m *Condition) GetExcludedPaths() []*istio_common_v1alpha1.StringMatch {
	if m != nil {
...
func (m *Condition) GetIncludedPaths() []*istio_common_v1alpha1.StringMatch {
	if m != nil {
...
type Policy struct {
	...
	// request.auth.issuer etc). JWT is used only when its condition matched. If
	// none of them are matched, request will be accepted as if there is no JWT defined.
	// If none of these methods pass, and origin_is_optional
...
func init() {
	...
	proto.RegisterType((*Condition)(nil), "istio.authentication.v1alpha1.Condition")
...
var fileDescriptorPolicy = []byte{
	...
	,
}
