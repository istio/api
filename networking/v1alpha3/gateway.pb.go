...
type Gateway struct {
	...
	Http                 *Gateway_HTTPOptions `protobuf:"bytes,3,opt,name=http,proto3" json:"http,omitempty"`
...
func (m *Gateway) GetHttp() *Gateway_HTTPOptions {
	if m != nil {
		return m.Http
	}
	return nil
...
// Set of HTTP related options that govern the gateway's behavior.
// Use these options to control http behavior, including trusted
// client IP address and headers.
type Gateway_HTTPOptions struct {
	// The number of additional ingress proxy hops from the right side of the
	// X-Xorwarded-For HTTP header to trust when determining the origin client’s
	// IP address.
	//
	// Gateways on cloud infrastructure are commonly placed behind at least
	// 1 external cloud load balancer (e.g. GCP or AWS Load Balancers), and
	// potentially further internal load balancers (e.g. Kubernetes Service).
	//
	// This setting affects the trusted client remote IP address, X-Forwarded-For
	// header, and X-Envoy-External-address and X-Envoy-Internal headers
	// for use cases including:
	// - traffic management e.g. consistent hash load balancing by IP
	//   address or X-Forwarded-For header
	// - policy enforcement e.g. security whitelist or blacklisting IP ranges
	//
	// For reference and examples of behavior,
	// see the following Envoy documentation:
	// https://www.envoyproxy.io/docs/envoy/latest/configuration/http_conn_man/headers#x-forwarded-for
	//
	// For example, a Gateway running behind 1 external cloud load balancer
	// and 1 internal load balancer would receive X-Forwarded-For in the format:
	//
	// ```
	// x-forwarded-for=<possible untrusted ips>, <client ip>, <external LB ip>, <internal LB ip>
	// ```
	//
	// The following configuration with `xff_num_trusted_hops: 2` would
	// set the trusted client source IP to `<client ip>`, as desired,
	// which is 2 hops from the right side of the X-Forwarded-For header.
	//
	// Without this configuration, the trusted client source IP would
	// be set to `<internal LB ip>`, which is not useful nor accurate.
	//
	// ```yaml
	// apiVersion: networking.istio.io/v1alpha3
	// kind: Gateway
...
func (m *Gateway_HTTPOptions) Reset()         { *m = Gateway_HTTPOptions{} }
func (m *Gateway_HTTPOptions) String() string { return proto.CompactTextString(m) }
func (*Gateway_HTTPOptions) ProtoMessage()    {}
func (*Gateway_HTTPOptions) Descriptor() ([]byte, []int) {
	return fileDescriptor_067d98d02f84cc0b, []int{0, 1}
}
func (m *Gateway_HTTPOptions) XXX_Unmarshal(b []byte) error {
	return m.Unmarshal(b)
}
func (m *Gateway_HTTPOptions) XXX_Marshal(b []byte, deterministic bool) ([]byte, error) {
	if deterministic {
		return xxx_messageInfo_Gateway_HTTPOptions.Marshal(b, m, deterministic)
	} else {
		b = b[:cap(b)]
		n, err := m.MarshalTo(b)
		if err != nil {
...
func (m *Gateway_HTTPOptions) XXX_Merge(src proto.Message) {
	xxx_messageInfo_Gateway_HTTPOptions.Merge(m, src)
}
func (m *Gateway_HTTPOptions) XXX_Size() int {
	return m.Size()
}
func (m *Gateway_HTTPOptions) XXX_DiscardUnknown() {
	xxx_messageInfo_Gateway_HTTPOptions.DiscardUnknown(m)
}

var xxx_messageInfo_Gateway_HTTPOptions proto.InternalMessageInfo

func (m *Gateway_HTTPOptions) GetXffNumTrustedHops() uint32 {
	if m != nil {
...
func init() {
	...
	proto.RegisterType((*Gateway_HTTPOptions)(nil), "istio.networking.v1alpha3.Gateway.HTTPOptions")
...
var fileDescriptor_067d98d02f84cc0b = []byte{
	...
	,
}
...
func (m *Gateway) MarshalTo(dAtA []byte) (int, error) {
	...
	if m.Http != nil {
		dAtA[i] = 0x1a
		i++
		i = encodeVarintGateway(dAtA, i, uint64(m.Http.Size()))
		n1, err1 := m.Http.MarshalTo(dAtA[i:])
		if err1 != nil {
			return 0, err1
		}
		i += n1
...
func (m *Gateway_HTTPOptions) Marshal() (dAtA []byte, err error) {
	size := m.Size()
	dAtA = make([]byte, size)
	n, err := m.MarshalTo(dAtA)
	if err != nil {
...
func (m *Gateway_HTTPOptions) MarshalTo(dAtA []byte) (int, error) {
	var i int
	_ = i
	var l int
	_ = l
	if m.XffNumTrustedHops != 0 {
		dAtA[i] = 0x8
		i++
		i = encodeVarintGateway(dAtA, i, uint64(m.XffNumTrustedHops))
	}
	if m.XXX_unrecognized != nil {
		i += copy(dAtA[i:], m.XXX_unrecognized)
	}
...
func (m *Server) MarshalTo(dAtA []byte) (int, error) {
	...
	if m.Tls != nil {
		...
		n3, err3 := m.Tls.MarshalTo(dAtA[i:])
		if err3 != nil {
			return 0, err3
		}
		i += n2
...
func (m *Gateway_HTTPOptions) Size() (n int) {
	if m == nil {
		return 0
	}
	var l int
	_ = l
	if m.XffNumTrustedHops != 0 {
		n += 1 + sovGateway(uint64(m.XffNumTrustedHops))
	}
	if m.XXX_unrecognized != nil {
		n += len(m.XXX_unrecognized)
	}
...
func (m *Gateway_HTTPOptions) Unmarshal(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
