...
// provides a template for `WorkloadEntry`, similar to the relationship between `Deployment` and `Pod`.
// `WorkloadGroup` has no relationship to `ServiceEntry` and says nothing about a service or host.
//
// <!-- crd generation tags
...
// `WorkloadEntryTemplate` describes the data a `WorkloadGroup` should have when created from a template.
