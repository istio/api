...
const (
	...
	// forwarded to the specified egress gateway. Applicable only for
	// HTTP and TLS traffic.
	OutboundTrafficPolicy_ALLOW_ANY_VIA_EGRESS_GATWEAY OutboundTrafficPolicy_Mode = 2
)

var OutboundTrafficPolicy_Mode_name = map[int32]string{
	0: "REGISTRY_ONLY",
	1: "ALLOW_ANY",
	2: "ALLOW_ANY_VIA_EGRESS_GATWEAY",
}

var OutboundTrafficPolicy_Mode_value = map[string]int32{
	"REGISTRY_ONLY":                0,
	"ALLOW_ANY":                    1,
	"ALLOW_ANY_VIA_EGRESS_GATWEAY": 2,
}
...
type OutboundTrafficPolicy struct {
	...
	// The hostname of the egress gateway to which the traffic should be
	// forwarded to. The hostname should be in the namespace/fqdn
	// format, corresponding to a gateway service in the mesh. Traffic
	// to the egress gateway from the sidecar will automatically use
	// Istio mutual TLS. This field is applicable only when the mode is
	// set to ALLOW_ANY_VIA_EGRESS_GATEWAY. If omitted, defaults to
	// istio-egressgateway.istio-system.svc.cluster.local when
	// applicable.
	EgressGatewayHost string `protobuf:"bytes,2,opt,name=egress_gateway_host,json=egressGatewayHost,proto3" json:"egress_gateway_host,omitempty"`
	// The port to which the traffic should be forwarded to. Defaults to
	// 15443 if omitted.
	EgressGatewayPort    uint32   `protobuf:"varint,3,opt,name=egress_gateway_port,json=egressGatewayPort,proto3" json:"egress_gateway_port,omitempty"`
	XXX_NoUnkeyedLiteral struct{} `json:"-"`
	XXX_unrecognized     []byte   `json:"-"`
	XXX_sizecache        int32    `json:"-"`
}
...
func (m *OutboundTrafficPolicy) GetEgressGatewayHost() string {
	if m != nil {
		return m.EgressGatewayHost
	}
	return ""
}

func (m *OutboundTrafficPolicy) GetEgressGatewayPort() uint32 {
	if m != nil {
		return m.EgressGatewayPort
	}
	return 0
}
...
var fileDescriptor_b5c11342f04ad3d1 = []byte{
	...
	,
}
...
func (m *OutboundTrafficPolicy) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	if m.EgressGatewayPort != 0 {
		i = encodeVarintSidecar(dAtA, i, uint64(m.EgressGatewayPort))
		i--
		dAtA[i] = 0x18
	}
	if len(m.EgressGatewayHost) > 0 {
		i -= len(m.EgressGatewayHost)
		copy(dAtA[i:], m.EgressGatewayHost)
		i = encodeVarintSidecar(dAtA, i, uint64(len(m.EgressGatewayHost)))
		i--
		dAtA[i] = 0x12
	}
	if m.Mode != 0 {
...
func (m *OutboundTrafficPolicy) Size() (n int) {
	...
	l = len(m.EgressGatewayHost)
	if l > 0 {
		n += 1 + l + sovSidecar(uint64(l))
	}
	if m.EgressGatewayPort != 0 {
		n += 1 + sovSidecar(uint64(m.EgressGatewayPort))
	}
	if m.XXX_unrecognized != nil {
		n += len(m.XXX_unrecognized)
	}
...
func (m *OutboundTrafficPolicy) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 2:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field EgressGatewayHost", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.EgressGatewayHost = string(dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		case 3:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field EgressGatewayPort", wireType)
			}
			m.EgressGatewayPort = 0
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return ErrIntOverflowSidecar
				}
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.EgressGatewayPort |= uint32(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
