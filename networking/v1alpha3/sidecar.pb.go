...
type InboundTrafficPolicy_IstioPeerAuthentication int32

const (
	// Inherits from the parent.
	InboundTrafficPolicy_UNSET InboundTrafficPolicy_IstioPeerAuthentication = 0
	// Treat all traffic as traffic coming from endpoints without
	// sidecars, i.e. they could be plain text or using custom TLS
	// certificates. Use this mode when defining custom TLS
	// certificates for TLS termination at the sidecar.
	InboundTrafficPolicy_DISABLE InboundTrafficPolicy_IstioPeerAuthentication = 1
	// Allow traffic from both sidecars (using Istio mutual TLS based
	// authentication) as well as non-sidecar endpoints. Traffic from
	// non-sidecar endpoints will be forwarded as is, with no TLS
	// termination.
	InboundTrafficPolicy_OPTIONAL InboundTrafficPolicy_IstioPeerAuthentication = 2
	// Allow only traffic from sidecars or endpoints using Istio
	// mutual TLS based authentication.
	InboundTrafficPolicy_REQUIRED InboundTrafficPolicy_IstioPeerAuthentication = 3
)

var InboundTrafficPolicy_IstioPeerAuthentication_name = map[int32]string{
	0: "UNSET",
	1: "DISABLE",
	2: "OPTIONAL",
	3: "REQUIRED",
...
var InboundTrafficPolicy_IstioPeerAuthentication_value = map[string]int32{
	"UNSET":    0,
	"DISABLE":  1,
	"OPTIONAL": 2,
	"REQUIRED": 3,
...
func (x InboundTrafficPolicy_IstioPeerAuthentication) String() string {
	return proto.EnumName(InboundTrafficPolicy_IstioPeerAuthentication_name, int32(x))
}

func (InboundTrafficPolicy_IstioPeerAuthentication) EnumDescriptor() ([]byte, []int) {
...
type Sidecar struct {
	...
	// Configuration for the inbound traffic policy, specifically Istio
	...
	InboundTrafficPolicy *InboundTrafficPolicy `protobuf:"bytes,5,opt,name=inbound_traffic_policy,json=inboundTrafficPolicy,proto3" json:"inbound_traffic_policy,omitempty"`
...
func (m *Sidecar) GetInboundTrafficPolicy() *InboundTrafficPolicy {
	if m != nil {
		return m.InboundTrafficPolicy
	}
	return nil
}
...
type IstioIngressListener struct {
	...
	// Settings that determine how the peer should be authenticated, as
	// well as settings to terminate application level TLS using user
	// specified certificates.
	TrafficPolicy        *InboundTrafficPolicy `protobuf:"bytes,5,opt,name=traffic_policy,json=trafficPolicy,proto3" json:"traffic_policy,omitempty"`
	XXX_NoUnkeyedLiteral struct{}              `json:"-"`
	XXX_unrecognized     []byte                `json:"-"`
	XXX_sizecache        int32                 `json:"-"`
}
...
func (m *IstioIngressListener) GetTrafficPolicy() *InboundTrafficPolicy {
	if m != nil {
...
// `InboundTrafficPolicy` sets the default behavior of the sidecar for
// authenticating inbound traffic to the application.
//
// ** NOTE 1**: InboundTrafficPolicy property, if not specified, is
// inherited as follows:
//
// 1. Ingress listener inherits the property, if not set, from the
// workload-level transport security property.
//
// 2. A workload-specific Sidecar inherits the property, if not set,
// from a namespace-wide Sidecar if present.
//
// 2. The namespace-wide Sidecar (i.e. the one without a workload
...
// if present.
//
// ** NOTE 2**: All sidecars default to using Istio mutual TLS based
...
// the appropriate TLS mode for their outbound connections. _No action
// is necessary if the peer authentication mode is disabled using a
// namespace-wide or global default Sidecar_.
//
// ** NOTE 3**: InboundTrafficPolicy settings for
// `istioPeerAuthentication` take precedance over the deprecated API
...
type InboundTrafficPolicy struct {
	// Determines the method to use to authenticate the remote
	...
	// terminating the application's TLS traffic using certificates
	// provided in the `tls` option below.
	IstioPeerAuthentication InboundTrafficPolicy_IstioPeerAuthentication `protobuf:"varint,1,opt,name=istio_peer_authentication,json=istioPeerAuthentication,proto3,enum=istio.networking.v1alpha3.InboundTrafficPolicy_IstioPeerAuthentication" json:"istio_peer_authentication,omitempty"`
	...
	// `istioPeerAuthentication` is set to `DISABLE`.
	Tls                  *Server_TLSOptions `protobuf:"bytes,2,opt,name=tls,proto3" json:"tls,omitempty"`
	XXX_NoUnkeyedLiteral struct{}           `json:"-"`
	XXX_unrecognized     []byte             `json:"-"`
	XXX_sizecache        int32              `json:"-"`
}

func (m *InboundTrafficPolicy) Reset()         { *m = InboundTrafficPolicy{} }
func (m *InboundTrafficPolicy) String() string { return proto.CompactTextString(m) }
func (*InboundTrafficPolicy) ProtoMessage()    {}
func (*InboundTrafficPolicy) Descriptor() ([]byte, []int) {
	return fileDescriptor_b5c11342f04ad3d1, []int{5}
}
func (m *InboundTrafficPolicy) XXX_Unmarshal(b []byte) error {
	return m.Unmarshal(b)
}
func (m *InboundTrafficPolicy) XXX_Marshal(b []byte, deterministic bool) ([]byte, error) {
	if deterministic {
		return xxx_messageInfo_InboundTrafficPolicy.Marshal(b, m, deterministic)
	} else {
		b = b[:cap(b)]
		n, err := m.MarshalToSizedBuffer(b)
		if err != nil {
...
func (m *InboundTrafficPolicy) XXX_Merge(src proto.Message) {
	xxx_messageInfo_InboundTrafficPolicy.Merge(m, src)
}
func (m *InboundTrafficPolicy) XXX_Size() int {
	return m.Size()
}
func (m *InboundTrafficPolicy) XXX_DiscardUnknown() {
	xxx_messageInfo_InboundTrafficPolicy.DiscardUnknown(m)
}

var xxx_messageInfo_InboundTrafficPolicy proto.InternalMessageInfo

func (m *InboundTrafficPolicy) GetIstioPeerAuthentication() InboundTrafficPolicy_IstioPeerAuthentication {
	if m != nil {
		return m.IstioPeerAuthentication
	}
	return InboundTrafficPolicy_UNSET
}

func (m *InboundTrafficPolicy) GetTls() *Server_TLSOptions {
	if m != nil {
		return m.Tls
	}
	return nil
...
func init() {
	...
	proto.RegisterEnum("istio.networking.v1alpha3.InboundTrafficPolicy_IstioPeerAuthentication", InboundTrafficPolicy_IstioPeerAuthentication_name, InboundTrafficPolicy_IstioPeerAuthentication_value)
	...
	proto.RegisterType((*InboundTrafficPolicy)(nil), "istio.networking.v1alpha3.InboundTrafficPolicy")
}
...
var fileDescriptor_b5c11342f04ad3d1 = []byte{
	...
	,
}
...
func (m *Sidecar) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	if m.InboundTrafficPolicy != nil {
		{
			size, err := m.InboundTrafficPolicy.MarshalToSizedBuffer(dAtA[:i])
			if err != nil {
				return 0, err
			}
			i -= size
...
func (m *InboundTrafficPolicy) Marshal() (dAtA []byte, err error) {
	size := m.Size()
	dAtA = make([]byte, size)
	n, err := m.MarshalToSizedBuffer(dAtA[:size])
	if err != nil {
...
func (m *InboundTrafficPolicy) MarshalTo(dAtA []byte) (int, error) {
	size := m.Size()
	return m.MarshalToSizedBuffer(dAtA[:size])
}

func (m *InboundTrafficPolicy) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	i := len(dAtA)
	_ = i
	var l int
	_ = l
	...
	if m.IstioPeerAuthentication != 0 {
		i = encodeVarintSidecar(dAtA, i, uint64(m.IstioPeerAuthentication))
		i--
		dAtA[i] = 0x8
	}
	return len(dAtA) - i, nil
...
func (m *InboundTrafficPolicy) Size() (n int) {
	if m == nil {
		return 0
	}
	var l int
	_ = l
	if m.IstioPeerAuthentication != 0 {
		n += 1 + sovSidecar(uint64(m.IstioPeerAuthentication))
	}
	if m.Tls != nil {
		l = m.Tls.Size()
		n += 1 + l + sovSidecar(uint64(l))
...
func (m *Sidecar) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 5:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field InboundTrafficPolicy", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			if m.InboundTrafficPolicy == nil {
				m.InboundTrafficPolicy = &InboundTrafficPolicy{}
			}
			if err := m.InboundTrafficPolicy.Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		default:
...
func (m *IstioIngressListener) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 5:
			...
			if m.TrafficPolicy == nil {
				m.TrafficPolicy = &InboundTrafficPolicy{}
			}
...
func (m *InboundTrafficPolicy) Unmarshal(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		...
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: InboundTrafficPolicy: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: InboundTrafficPolicy: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field IstioPeerAuthentication", wireType)
			}
			m.IstioPeerAuthentication = 0
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return ErrIntOverflowSidecar
				}
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.IstioPeerAuthentication |= InboundTrafficPolicy_IstioPeerAuthentication(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
