...
type VirtualService struct {
	...
	// The names of gateways and sidecars that should apply these rules. A
	// single VirtualService can be used for sidecars inside the mesh as well
	// as for one or more gateways. The reserved word `mesh` is used to apply
	// the rules to all sidecars in the mesh. If this field is omitted, `mesh`
	// is implied by default. If a list of gateway names is provided, the
	// rules will apply only to those gateways unless `mesh` is also included
	// in the list. Gateways defined in a different namespace can be selected
	// by prefixing the gateway name with `namespace/`.
	//
