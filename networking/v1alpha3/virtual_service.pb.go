...
// a host is addressed. Here are a few terms useful to define in the
// context of traffic routing.
//
...
// actual choice of the version is determined by the proxy/sidecar,
// enabling the application code to decouple itself from the evolution of
// dependent services.
//
// Each routing rule in the `VirtualService` defines matching criteria for
// traffic of a specific protocol. If the traffic is matched, then it is
// sent to a named destination service (or subset/version of it) defined in
// the registry.
//
// The source of traffic can also be matched in a routing rule. This allows
// routing to be customized for specific client contexts.
//
