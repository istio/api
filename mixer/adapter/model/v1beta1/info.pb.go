...
type Info struct {
	...
	//
	// If true, during configuration time, Mixer would call the `InfrastructureBackend`' rpcs
	// to validate and pass the handler configuration. And during request-time, Mixer would not pass the handler
	// configuration, and would only pass the template-specific Instance payload using the template-specific handle service (Example
	...
	// If `session_based` is false, Mixer would not expect backend to implement `InfrastructureBackend` service, and would
	// only communicate with backends during request-time through the template-specific handle service. Without
	// `InfrastructureBackend` service, Mixer would pass the handler configuration on each call during request-time.
