...
type PluginConfig struct {
	...
	// entry will be generated. If there are any type errors, the CEL expression is evaluated as false.
	// More details on type checking can be found https://kubernetes.io/docs/reference/using-api/cel/#type-checking.
	// A common error is referring to a non-existent field in the log entry.
	// It's crucial to note that in Envoy, the fields that appear in access log entries can vary. This variation is influenced by several factors,
	// including the protocol in use (such as HTTP or TCP), the applied filters, and the specific configuration of the Envoy instance.
	// Therefore, when using CEL expressions for filtering access logs, it's essential to ensure that these expressions accurately refer to existing fields in the log entry.
	// The has() macro in CEL may be used in CEL expressions to check if a field is accessible before attempting to access the field's value.
	// There's also the CEL Playground https://playcel.undistro.io/ where CEL expressions can be quickly tested.
